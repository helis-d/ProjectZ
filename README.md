...
1. **Buy Phase (20 seconds):** Players use credits earned from kills (200 cr), objectives (300 cr), round wins (3000 cr), and round losses (base 1900 cr scaling with losing streaks) to purchase weapons, armor, and abilities natively through the `BuyMenuUI`. Economy maximum is capped at 9000 credits for Ranked matches. Defenders may purchase a **defuse kit** during buy phase (`BuyMenuUI.RequestBuyDefuseKit()`); price is set on the component (align with the GDD buy grid when published). This enables the GDD **3.5s** defuse time versus **7.0s** without a kit.
