...
**Design / product:** Mode shipping tiers, mastery tuning, and ultimate counterplay are documented in [`DESIGN_PILLARS.md`](DESIGN_PILLARS.md), [`HERO_ULTIMATE_PIPELINE.md`](HERO_ULTIMATE_PIPELINE.md), and [`COMPETITIVE_INTEGRITY_PASS.md`](COMPETITIVE_INTEGRITY_PASS.md). Code: `GameModeProductInfo`, `WeaponMasteryManager.masteryHandlingStrength`. Weapon vs mastery ownership: [`WEAPON_SYSTEMS.md`](WEAPON_SYSTEMS.md).

**Tests:** Edit Mode tests under `Assets/Tests/EditMode` (e.g. `LevelMultipliersBlendTests`) guard mastery blend math without a scene.
